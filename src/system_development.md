...
In the context of the nature of user and its environment, an app that brings the information about the recently occurred major events, upcoming music events near by, newly emerging things, and surprising and shocking events in the music industry along with the ability to bring the relevant bring interesting fun fact about the music industry history that user would want to explore, such that it helps user to spend his leisure time with satisfaction of being able to explore area of his interest.
...
Considering the philosophy of single responsibility principle for the development implementation, we are moving forward with the microservices architecture. Although, we only have two services however, considering the rapid demand of the users and aggressive feedback loop which we want to run after launch we would want the teams to aggressively work on different features separately while ensuring the scalability.



1. For now, our services do not interact with each other. However, we will assign them separate resources.
2. To ensure the reliability of the services (especially speed response \& logging conversation in real-time without loosing history) we will use AWS SQS for queuing the task.
3. Docker containerization for stable runtime environment of application.
...
1. **As of now, I have the clarity on how the development \& end to end system should work. Now, I just need to come up with the system architecture to ensure system so, that I can stay clear about what end-to-end system is supposed to look like.**
2. **After that, I need to come up with the feature-level architecture so, that I can start implementing it and as the implementation progresses the deployment and system comes into place along the way.**
3. **In this way, we will have feature and system development alongside, and I will get a chance to develop the end to end product on the go.** 
4. **For now, we will keep the Front-end part very simple \& limited to less features because that is not the goal. It is there just to complete the system.**
